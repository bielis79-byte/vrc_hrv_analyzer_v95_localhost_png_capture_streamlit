...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v9.5", layout="wide")
...
def _add_subplot_side_legend(fig, row, col, items, title=None, x_pad=0.012, y_pad=0.012):
    """
    Añade una leyenda manual en el margen derecho de un subplot.

    items: lista de tuplas (label, color, symbol)
    Usa coordenadas paper y queda fuera del área de datos.
    """
    try:
        # Detectar ejes del subplot
        cell = fig._grid_ref[row - 1][col - 1][0]
        xaxis_name = "xaxis" + (str(cell.trace_kwargs["xaxis"][1:]) if cell.trace_kwargs["xaxis"] != "x" else "")
        yaxis_name = "yaxis" + (str(cell.trace_kwargs["yaxis"][1:]) if cell.trace_kwargs["yaxis"] != "y" else "")

        xdom = getattr(fig.layout, xaxis_name).domain
        ydom = getattr(fig.layout, yaxis_name).domain
        x0 = min(xdom[1] + x_pad, 0.985)
        y0 = ydom[1] - y_pad
        ...
        for i, (label, color, symbol) in enumerate(items):
            fig.add_annotation(
                x=x0,
                y=y0 - i * 0.028,
                xref="paper",
                yref="paper",
                text=f"<span style='color:{color}'>{symbol}</span> {label}",
                showarrow=False,
                align="left",
                xanchor="left",
                yanchor="top",
    ...
    for idx, param in enumerate(params):
        ...
        if one_phase:
            ...
            _add_subplot_side_legend(
                fig, rr, cc,
                [(f"{param} columnas", color, "■"), ("Tendencia suavizada", color, "━")],
                title=param
            )
...
st.title("VRC / HRV RRi Analyzer Pro v9.5")
    ...
    if long_df.empty:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            st.caption(
                ...
                "Si PNG falla en Streamlit Cloud, usa convert_html_to_png.py. Si quieres capturar la app local http://localhost:8501/, usa capture_streamlit_localhost_png.py."
            )
